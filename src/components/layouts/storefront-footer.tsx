...
function TrustRow({ stats }: { stats: StorefrontStats }) {
  ...
  const items = [
    { icon: CreditCard, label: theme.trust_line_payments },
    { icon: Clock, label: theme.trust_line_start },
    { icon: Star, label: ratingLabel },
    { icon: MessageCircle, label: theme.trust_line_support },
  ];
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          <div className="md:col-span-1">
            <Logo size="md" href="/" />
            <p className="mt-3 text-sm text-[var(--text-muted)] leading-relaxed">{theme.footer_brand_blurb}</p>
            <p className="mt-4 text-xs text-[var(--text-muted)]">{theme.footer_brand_note}</p>
          </div>

          {footerColumns.map((col) => (
            <div key={col.title}>
              <h3 className="text-xs font-semibold text-[var(--color-primary)]/80 uppercase tracking-widest mb-4">{col.title}</h3>
