...
export async function POST(req: NextRequest) {
  ...
  type AdminTable = { insert: (v: unknown) => Promise<unknown>; update: (v: unknown) => { eq: (k: string, v: string) => Promise<unknown> } };

  // Save AI response as message
  await (admin.from("ticket_messages") as unknown as AdminTable).insert({
    ticket_id: ticketId,
    sender_id: null,
  ...
  // Update ticket
  await (admin.from("tickets") as unknown as AdminTable).update({
    is_ai_handled: true,
    ai_confidence: aiResponse.confidence,
    status: aiResponse.shouldEscalate ? "in_progress" : "awaiting_reply",
